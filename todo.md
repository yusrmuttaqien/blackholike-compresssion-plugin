...
## Phase 7 — Compression stats & progress in the chat status line  ✅ DONE

Live compression stats (option A) and stage-level progress (option B) in the
status line above the assistant response. No new valves.

### [x] 7.1 i18n (`src/i18n.py`)
...
- Both locales (en-US, zh-CN).

### [x] 7.2 Stats (A)
- Pre-compression usage line (`src/compression.py`): loads the previous
  `chat_summary` record and, when one exists, appends the tokens/messages
  accumulated since the last summary (best-effort; the base line survives any
  error). One cheap DB read, only on the path where the line is shown.
- Post-compression usage line (`src/summarize.py`): appends
  `pre_compression_tokens - token_count` as "Saved ~N tokens".
  `_generate_summary_async` gained a `pre_compression_tokens` parameter,
  passed from `_check_and_generate_summary_async`.

### [x] 7.3 Stage shimmers (B)
- Stage 1 before the LLM call: "Compressing conversation history (N messages)..."
  (N = the actual message count being compressed, protected-prefix excluded).
- Stage 2 (pre-existing): "Generating context summary in background...".
- Stage 3 before the DB write: "Saving context summary...".
- All `done: false`; the sequence resolves into the static post-completion
  statuses. Safe in OWUI: the expanded status history forces `done` on older
  entries, so finished stages do not shimmer.

### [x] 7.4 Verification
- `ast.parse` OK; reproducible build (4333 lines); exactly 3 `done: false`
  status emissions in the built file.
- Behavioral tests on the extracted `_check_and_generate_summary_async`: no
  record -> plain line; record -> correct delta + `pre_compression_tokens`
  passed through; overshooting `prev_count` -> no delta.
- i18n smoke: all 4 new keys format correctly in en-US + zh-CN; unknown
  locales fall back to en-US.
- README gained a "Chat status line" section.
